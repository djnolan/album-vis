...
export default function HomeScreen({ userAlbums = [], paletteOverrides = {}, onSelectAlbum, onCreateClick }) {
  ...
  return (
    <div className="bg-surface-0 min-h-screen">
      ...
      {/* Fixed bottom bar */}
      <div
        className="fixed bottom-0 left-0 right-0 flex items-center px-6 py-5 bg-surface-1 border-t border-border"
        style={{ boxShadow: '0 -4px 16px rgba(0,0,0,0.4)' }}
      >
        <button
          onClick={onCreateClick}
          className="w-full py-3 bg-accent text-surface-0 font-sans text-lg font-bold leading-none rounded-sm"
        >
          CREATE +
        </button>
      </div>
